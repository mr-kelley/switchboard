...
## Active Work
- **Milestone:** Fleet (v6). Sprint 21 (Issue #47) live-verified end-to-end on multiple Linux targets: paired daemons persist across client restarts; parallel daemons work; sessions on valid cwd accept input and stream output. rc.2–rc.6 (PR #50 + PR #51 merged) tightened tarball bundling, remote-daemon binding, install-service restart, cwd validation, and error surfacing.
- **In progress:** rc.7 — bundle Node.js runtime in the daemon tarball. Live-test revealed a shared-host target with no user-installable Node. `scripts/build-daemon-tarball.sh` now downloads and embeds Node 20 LTS (cached under `.cache/node/`); `bin/node` lands inside the tarball at `switchboard-daemon/bin/node`. Provisioner drops the Node probe; systemd unit's ExecStart uses the bundled binary. Tarball grew from 176 KB → 35 MB; on-target install now works with zero Node prerequisites. Governed by DEC-000009 (supersedes DEC-000008's `assume Node on target` posture).
...
- DEC-000008: Daemon distributable shape — gzip tarball (~176 KB) with bundled `ws` + `node-pty` (linux-x64 prebuild); assumes Node 20 on target. Not a standalone binary in v1. (Node-on-target assumption reversed by DEC-000009 after live-test hit a shared-host target with no Node.)
- DEC-000009: Bundle Node.js runtime in the daemon tarball; drop the Node-on-target assumption. Tarball ships `bin/node` (Node 20 LTS, ~35 MB gzipped). Remote install now works on targets with no Node available.
