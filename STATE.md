...
## Active Work
- **Milestone:** Fleet (v6) — starting. rc.12 (retirement pass, PR #46) merged and self-install verified on rc.12; Flow II fully complete.
- **In progress:** Sprint 21 planning — remote daemon provisioning over SSH (Issue #47). Sprint file drafted at `sprints/fleet/21-remote-daemon-provisioning.md` covering: shell-out SSH transport, daemon tarball distributable (assumes Node ≥ 20 on target), 10-step install state machine (`test → probe → check → upload → extract → systemd unit + enable --now → wait ready → read pairing code → auto-pair → cleanup`), new "Add remote daemon" modal, small daemon change to write pairing code to `~/.local/state/switchboard/pairing-code.txt`. Two DEC entries flagged: DEC-000007 (SSH transport shape) and DEC-000008 (daemon distributable shape).
- **Next up:** Reach agreement on the sprint scope, then implement. Sprint may split into 21a/21b/21c if the surface is too large for one PR — natural cut points documented in the sprint file.
