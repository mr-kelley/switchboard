...
- **In progress:** rc.10 — window-icon fix. `new BrowserWindow(...)` in `src/main/main.ts` had no `icon` option, and `build/icon.png` is not bundled into `dist/**` at build time, so window managers on Linux fall back to the generic Electron icon. Fix embeds the icon as a base64 PNG data URL (`src/main/app-icon.ts`, generated from `build/icon.png` by `scripts/gen-app-icon.py` at 256×256) and passes it via `nativeImage.createFromDataURL`. Mirrors the tray-icons embedding pattern.
- **Next up:** After rc.10 merges — Sprint 21 (v6 Fleet Sprint 1: remote daemon provisioning) once its GitHub issue is filed.
