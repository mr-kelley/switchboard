...
## Active Work
- **Milestone:** Flow II (v4) — complete. All observables shipped and live-verified on Ubuntu 24.04 GNOME 2026-07-31 (cross-group drag remains untested pending v6 remote provisioning — single-daemon limitation, not a code gap).
- **In progress:** rc.12 — retirement pass. Removes `.deb` and `.snap` targets from `package.json` (linux.target, deb + snap config blocks, `dist:deb` / `dist:snap` npm scripts, orphaned `build/after-install.sh`). AppImage is the sole Linux distribution from this release forward; DEC-000005's self-install (verified in rc.11) covers desktop integration across all distros. Governed by DEC-000006.
- **Next up:** Sprint 21 (v6 Fleet Sprint 1: remote daemon provisioning) — the sprint that finally lets users set up new servers from within Switchboard and unblocks cross-host session groups. Blocked on user filing the GitHub issue (governance).
